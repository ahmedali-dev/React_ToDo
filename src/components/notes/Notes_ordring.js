...
const NotesOrdring = ({ data, ...props }) => {
  ...
  const ordaringNote = (event = null) => {
    ...
    if (!event) {
      ...
      pagex =
        event.currentTarget.innerWidth > 1200
          ? 1200
    ...
    const NoteWidth = 270;
    const NotesColumn =
      Math.floor(pagex / NoteWidth) > 5 ? 5 : Math.floor(pagex / NoteWidth);
  ...
  const css = (Class = "") =>
    Class.length > 0 ? style[`NotesRows_${Class}`] : style.NotesRows;
  return (
    <div className={css()}>
      {rows &&
        rows.map((row) => {
          ...
          return (
            <div className={css("row")}>
              {row.map((note) => (
                <Note id={note._id} title={note.title} note={note.note} />
              ))}
            </div>
          );
        })}
