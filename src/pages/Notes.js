...
const dummyNotes = [
  ...
  },
  {
    ...
    content:
      "Remember to do 30 minutes of cardio and 15 minutes of strength training.",
    date: "2023-06-14",
  },
  {
    id: 7,
...
const Notes = ({ add }) => {
  const css = (Class = "") =>
    Class.length > 0 ? style[`Notes_${Class}`] : style.Notes;
  ...
  let getNote = null;
  if (params.id) {
    getNote = selected.notes.find((note) => note._id == params.id);
    console.log(getNote);
  }
  ...
  return (
    <div className={css()}>
      ...
      <Link to={"/notes/add"} className={css("addNote")}>
        <Add />
